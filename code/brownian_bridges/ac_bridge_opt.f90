...
!Computes optimal forces in softened Brownian bridge, Gaussian basis, uses Actor critic algorithm (ac)
...
!Learning
double precision, parameter:: alpha=1.0,alphaV=0.05 !learning rates
...
double precision:: F1,F1old !force in new and previous timestep
double precision:: delu(Mx,Mt),deluold(Mx,Mt) !contains partial derivative of force w.r.t. parameters
...
double precision:: delq(Mx,Mt) !malliavin weight jump in every timestep
...
double precision:: gradV(Mvx,Mvt),gradVold(Mvx,Mvt) !gradient of value function
...
integer, parameter:: deln=100 !number of timesteps in n-step temporal difference !n=100 when \tau=0.1
double precision:: delqhist(deln+1,Mx,Mt),gradVhist(deln+1,Mvx,Mvt) !history of delq and gradV from deln timesteps before
...
do i=1,steps
    ...
    call force(Mx,Mt,Mvx,Mvt,x2,i,coeff,coeffV,F1,V,delu,gradV,gaussianx,gaussiant,steps,tau)
    ...
    rval=((delx/tau-F1)**2-(delx/tau)**2)/(4.0d0*D) *tau
    omega(3)=omega(3)+rval
    if (i==steps .and. abs(x2-xf)<1d-1) then !reached target region
        omega(2)=+lam*1
        rval=rval+omega(2)
    end if
    ...
    !store delq,V,gradV,rval history
