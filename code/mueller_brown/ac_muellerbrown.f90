...
!Computes optimal forces in potential with Mueller-Brown potential, Gaussian basis, uses Actor critic algorithm (ac)
...
do i=1,steps
    ...
    if (i==steps) then 
        if (x2(2)>0.7d0 .and. pot<-145) omega(2)=1 !reached target region
        
        !for quadratic initialization
        omega(4)=omega(4)-rval
        rval=rval-lam*(-0.5-x2(1))**2-lam*(1.5-x2(2))**2
        omega(4)=omega(4)+rval
