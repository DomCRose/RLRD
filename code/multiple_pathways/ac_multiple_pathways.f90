...
!Computes optimal forces in potential with multiple reaction pathways, Gaussian basis, uses Actor critic algorithm (ac)
!main program, manages mpi side of things, calls subroutine "traj"
program valuebaseline
use mpi
implicit none

double precision, parameter:: lam=500 !bias
...
!Learning
double precision, parameter:: alpha=1,alphaV=0.5 !learning rates !set alphaV=0 for mcr and !=0 for mcvb
...
double precision,dimension(2):: F1,F0,F1old !force in new and previous timestep
double precision:: delu(Mx,My,Mt,2),deluold(Mx,My,Mt,2) !contains partial derivative of force w.r.t. parameters, last rank is component
...
double precision:: delq(Mx,My,Mt,2) !jump in malliavin weight in every timestep
...
double precision:: V,Vold !value function
double precision:: gradV(Mvx,Mvy,Mvt),gradVold(Mvx,Mvy,Mvt) !gradient of value function
!value function uses the same gaussian basis as the force
double precision:: rval !return in every timestep
double precision:: deltd !temporal difference
integer, parameter:: deln=1 !number of timesteps in n-step temporal difference !n=1 when \tau=0.001
double precision:: delqhist(deln+1,Mx,My,Mt,2),gradVhist(deln+1,Mvx,Mvy,Mvt) !history of delq and gradV from deln timesteps before
double precision:: Vhist(deln+1),rvalhist(deln+1) !history of V and rval from deln timesteps before
double precision:: rvalcmlt !cumulated rval over deln steps
integer:: point, histpoint !array index for finding the current element and the element deln steps ago
...
do j=1,My
  gaussiany(j,1)=-1.5+(j-1)*(1.5-(-1.5))/(My-1)
end do
gaussiany(:,2)=((1.5-(-1.5))/(My-1)/2.0d0)**2 !variance
...
do i=1,steps
    ...
    !propagate malliavin weight and return related to this jump
    delq(:,:,:,1)=etavar*eta(1)*delu(:,:,:,1)/(2.0d0*D)
    delq(:,:,:,2)=etavar*eta(2)*delu(:,:,:,2)/(2.0d0*D)
    rval=sum(((delx/tau-F1)**2-(delx/tau-F0)**2)/(4.0d0*D)) *tau
    omega(3)=omega(3)+rval
    if (i==steps) then 
        if (x2(1)>0 .and. pot<0) omega(2)=1 !reached target region
        
        !for quadratic initialization
        omega(4)=omega(4)-rval
        rval=rval-lam*(1.11-x2(1))**2-lam*(0.0-x2(2))**2
        omega(4)=omega(4)+rval
        omega(1)=omega(3)+omega(4)
        
        !Or, for indicator optimization, comment out above quadratic init block and uncomment following line
        !rval=rval+lam*omega(2)  
        !omega(1)=lam*omega(2)+omega(3)              
    end if
    
    !accumulate rvalcmlt
    rvalcmlt=rvalcmlt+rvalhist(point)-rvalhist(histpoint) !with old point and histpoint
    
    !store delq,V,gradV,rval
    point=modulo(point,deln+1)+1 !next position in the array
    rvalhist(point)=rval
    delqhist(point,:,:,:,:)=delq
    gradVhist(point,:,:,:)=gradV
    Vhist(point)=V

    !move the history index
    histpoint=modulo(point,deln+1)+1
    
    if (i>deln) then!start calculating temporal difference and gradient integrals
        deltd=Vhist(point)+rvalcmlt-Vhist(histpoint)
        delomega=delomega+deltd*delqhist(histpoint,:,:,:,:)
        delV=delV+deltd*gradVhist(histpoint,:,:,:)
    end if

    
end do !trajectory propagation ends

!keep calculating gradient integrals for deln more timesteps
do i=steps+1,steps+deln
    !return and value function explicitly put to 0
    rval=0.0d0
    V=0.0d0
    
    !Accumulate rvalcmlt
    rvalcmlt=rvalcmlt+rvalhist(point)-rvalhist(histpoint)
    
    !store V,rval !delq and gradV are explicitly 0
    point=modulo(point,deln+1)+1 !next position in the array
    rvalhist(point)=rval
    Vhist(point)=V
    ...
    !continue calculating temporal difference and gradient integrals
    deltd=Vhist(point)+rvalcmlt-Vhist(histpoint)
    delomega=delomega+deltd*delqhist(histpoint,:,:,:,:)
    delV=delV+deltd*gradVhist(histpoint,:,:,:)
end do
...
double precision:: aa,bb,cc,dd,x,y
double precision:: pot
...
pot=4.0d0/3.0d0 *(4.0d0*aa*aa+2*bb*bb+cc*cc+dd*dd-2.0d0)
F0(1)=-(-64.0d0*aa*x+32.0d0*bb*x+16.0d0*cc*(x+y)+16.0d0*dd*(x-y))/3.0d0
F0(2)=-(-64.0d0*aa*y+16.0d0*cc*(x+y)-16.0d0*dd*(x-y))/3.0d0
