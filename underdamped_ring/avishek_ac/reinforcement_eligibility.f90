...
integer(kind=8), parameter:: steps=5d5+ithreshold !total trajectory distribution
...
!reinforcement stuff
double precision, parameter:: alphar=5d-5,alphapsi=1d-2,alphatheta=5d0,trace_lambda=0.5
!double precision, dimension
integer, parameter:: M3=5
double precision, dimension(2*M3+1):: Vparams, delV, trace_v
double precision, dimension(2*M):: dela, trace_a
...
!                rand2(j)=grnd()
    ...
    do k=1,M
        do j=1,N
            trace_a(k)=trace_lambda*trace_a(k)+etavar*eta(j)*delu(k,j)/(2.0d0*D)
            trace_a(k+M)=trace_lambda*trace_a(k+M)+etavar*eta(j)*delu(k+M,j)/(2.0d0*D)
        end do
    end do
    do k=1,M3
        do j=1,N
            trace_v(k)=trace_lambda*trace_v(k)+cos(k*x1(j)*wvnm)
            trace_v(k+M3)=trace_lambda*trace_v(k+M3)+sin(k*x1(j)*wvnm)
        end do
    end do
    trace_v(2*M3+1)=trace_lambda*trace_v(2*M3+1)+1
    Vparams(:)=Vparams(:)+alphapsi*deltai*trace_v(:)
    delV=0.0d0
    if (mod(i-ithreshold,int(1d0))==0) then
        a(:)=a(:)+alphatheta*deltai*trace_a(:)/1d0
